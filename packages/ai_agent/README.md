# AI Agent Runtime Snapshot

This directory is a source snapshot of the Vela AI Agent runtime used by the
R528S3-DshanPi submission. The canonical openvela project is still
`openvela/packages/ai_agent`; this copy is included so the contest repository
contains the Agent implementation and its product Skill for review and
reproduction.

The snapshot reflects the submission changes: MiMo `mimo-v2.5` support,
WebSocket/CLI interaction, the `vela-desk` Skill, the
`routine_schedule_update` tool, and removal of Agent-level online music tools.
Build integration is applied by
`contest2026_113_veladuxingke/patches/packages_ai_agent/vela-desk-runtime.patch`
from the openvela workspace root. Do not copy this directory over the synced
upstream project; apply the patch instead.
...
- 🔧 **ReAct Agent** — 30+ 内置工具，支持并行执行，自动推理-行动循环
...
- 📝 **Skills 系统** — Markdown 格式可扩展技能，11 个内置 Skill，对话即可创建新技能
...
| `mimo` | mimo-v2.5 | 小米 MiMo，支持文本、视觉和工具调用 |
| `openai` | gpt-4o | OpenAI |
| `claude` | claude-sonnet-4-20250514 | Anthropic Claude |
| `openrouter` | openrouter/hunter-alpha | OpenRouter 聚合 |

```bash
vela> set_llm mimo <MIMO_API_KEY>        # 配置 MiMo v2.5
vela> router_model 0 mimo-v2.5-pro       # 将已配置的 MiMo 切换为 Pro
vela> list_models --free                 # 列出可用模型（openrouter）
```

MiMo 请求按官方协议使用 `api-key` 请求头。`tp-` Key 自动使用 `token-plan-cn.xiaomimimo.com/v1`，其他 Key 使用 `api.xiaomimimo.com/v1`。为避免 v2.5 默认开启深度思考导致工具调用延迟，文本/Skill 请求显式传递 `thinking=disabled`、`temperature=1.0`、`top_p=0.95` 和 `max_completion_tokens=32768`。升级时会在官方 MiMo 端点上自动迁移持久化的 v2 旧模型名和与 Key 类型不匹配的 Base URL。
...
11 个内置 Skill，Markdown 格式，对话即可创建新技能：
...
| `feishu-test` | 飞书集成测试 |
| `vela-desk` | 桌面环境查询和三条每日模板日程管理 |

`vela-desk.md` 会随固件以版本标记安装到 `/data/agent/skills/`；保留旧数据分区升级
固件时，启动器只更新缺失或低版本的产品文件，并删除已下线的 `music-dj.md`，其他
自定义 Skill 不受影响。
...
| [内置工具](docs/tools.md) | 内置工具列表、Shell 安全策略 |
