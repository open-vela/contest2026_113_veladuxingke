...
/* Queue a chat message after the wire format has been normalized.  The
 * WebSocket client may identify a conversation in the JSON envelope; plain
 * text and JSON-string frames use the connection's default chat_id. */
static void ws_dispatch_chat(int fd, char* chat_id, const char* content,
    const char* requested_chat_id)
{
    if (!content || !content[0])
        return;

    pthread_mutex_lock(&s_clients_mtx);
    ws_client_t* client = find_by_fd_locked(fd);
    if (client) {
        if (requested_chat_id && requested_chat_id[0]) {
            strncpy(client->chat_id, requested_chat_id,
                sizeof(client->chat_id) - 1);
            client->chat_id[sizeof(client->chat_id) - 1] = '\0';
        }
        strncpy(chat_id, client->chat_id, 31);
        chat_id[31] = '\0';
    }
...
static void* client_thread(void* arg)
{
    ...
    while (s_running) {
        ...
        /* Parse the documented JSON envelope first.  For convenience, also
         * accept a JSON string or a raw text frame from simple WS clients. */
        cJSON* root = cJSON_Parse(frame_buf);
        const char* content = NULL;
        const char* requested_chat_id = NULL;
        ...
        if (root) {
            ...
            if (cJSON_IsString(type_item)
                ...
                if (cJSON_IsString(cid_item))
                    requested_chat_id = cid_item->valuestring;
            } else if (cJSON_IsString(root)) {
                /* Some generic clients JSON-encode the text itself. */
                content = root->valuestring;
            }
        } else {
            /* Match the MQTT channel's plain-text behavior. */
            content = frame_buf;
            plain_text = true;
        }
        ...
        if (content && content[0]) {
            if (plain_text) {
                syslog(LOG_INFO, "[%s] WS plain-text frame from fd=%d (%d bytes)\n",
                    TAG, fd, n);
            }
            ws_dispatch_chat(fd, chat_id, content, requested_chat_id);
        } else if (!root) {
            syslog(LOG_WARNING, "[%s] Invalid empty WS frame from fd=%d\n",
                TAG, fd);
        }
        cJSON_Delete(root);
    }
