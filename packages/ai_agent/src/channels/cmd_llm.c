...
static const router_preset_t g_router_presets[] = {
    ...
    { "mimo", AGENT_LLM_MIMO_HOST, AGENT_LLM_MIMO_PATH,
        AGENT_LLM_MIMO_MODEL, 1 },
...
void cmd_set_llm(int argc, char** argv)
{
    if (argc < 2) {
        printf("Usage: set_llm <preset> [api_key]\n"
               ...
               "  mimo     - MiMo v2.5 (tp- key selects Token Plan)\n"
               "\n"
    ...
    const char* normalized_host = llm_normalize_host_for_key(
        backend.host, backend.api_key);
    if (normalized_host && strcmp(normalized_host, backend.host) != 0) {
        strncpy(backend.host, normalized_host, sizeof(backend.host) - 1);
        backend.host[sizeof(backend.host) - 1] = '\0';
    }
    ...
    printf("LLM backend: %s:%s%s (model: %s) [router slot 0]\n",
        backend.host, port, path, model ? model : "(unchanged)");
    if (api_key) {
...
void cmd_set_vision_llm(int argc, char** argv)
{
    ...
    static const struct {
        ...
        { "mimo", AGENT_LLM_MIMO_HOST, AGENT_LLM_MIMO_MODEL },
        { "openai", "api.openai.com", "gpt-4o" },
        { "qwen", "dashscope.aliyuncs.com", "qwen-vl-max" },
        { "glm", "open.bigmodel.cn", "glm-4v-flash" },
        { NULL, NULL, NULL }
    ...
    host = llm_normalize_host_for_key(host, api_key);
