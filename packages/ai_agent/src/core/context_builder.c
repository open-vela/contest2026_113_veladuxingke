...
int context_build_system_prompt(char *buf, size_t size)
{
    ...
    off += snprintf(buf + off, size - off,
        ...
        "- One-off reminders: get_current_time→calc epoch→cron_add(schedule_type=at).\n"
        "- IMPORTANT: routine_mgr has three fixed daily template slots "
        "(喝水/专注/通风) in /data/routine/status.json and "
        "/data/routine/config.json. They are NOT cron jobs. For today's "
        "reminders, unfinished routine items, or questions about where "
        "喝水/专注/通风 is configured, read /data/routine/status.json "
        "with run_shell and do not call cron_list. Use cron_list only when "
        "the user explicitly asks about ai_agent cron jobs or system timers.\n"
        "- Cron tool actions: set action+action_args in cron_add.\n"
        "- Routine template edits: read /data/routine/status.json first, "
        "then call routine_schedule_update with the exact slot ID; changes "
        "persist as the daily template. Never edit /data/routine/ with shell "
        "or file tools.\n"
        "- The cron scheduler's periodic `Check job` DEBUG line only means it\n"
        "inspected a job; it is not evidence that a reminder fired.\n"
        "- For current cron status, call cron_list in this turn; do not rely\n"
        "on conversation history or a cached answer. Before cron_remove,\n"
        "call cron_list and copy the exact current job ID; never guess IDs.\n"
        "If cron_remove reports an error or not found, say that nothing was\n"
        "removed and do not claim success.\n"
