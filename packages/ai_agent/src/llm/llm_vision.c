    ...
    size_t buf_size)
{
    ...
    add_completion_options(body, llm_host, AGENT_VISION_MAX_TOKENS);
    ...
    char* response_buf, size_t buf_size)
{
    ...
    const char* tokens_key = is_openai_compat_host(llm_host)
        ...
        : "max_tokens";
    char provider_options[128] = { 0 };
    if (is_mimo_host(llm_host)) {
        snprintf(provider_options, sizeof(provider_options),
            ",\"temperature\":%.1f,\"top_p\":%.2f,"
            "\"thinking\":{\"type\":\"%s\"}",
            AGENT_LLM_MIMO_TEMPERATURE, AGENT_LLM_MIMO_TOP_P,
    ...
    size_t hdr_max = strlen(hdr_fmt) + sizeof(model) + 32
        + strlen(tokens_key) + strlen(provider_options) + strlen(escaped_prompt)
        + strlen(mime_type) + 64;
    ...
    int off = snprintf(body, total, hdr_fmt,
        ...
        provider_options, escaped_prompt, mime_type);
    free(escaped_prompt);
