...
{
    return host && (strstr(host, "openai.com")
        || strstr(host, "openrouter.ai")
        || strstr(host, "xiaomimimo.com"));
}

bool is_mimo_host(const char* host)
{
    static const char domain[] = "xiaomimimo.com";
    if (!host)
        return false;

    size_t host_len = strlen(host);
    size_t domain_len = sizeof(domain) - 1;
    if (host_len == domain_len)
        return strcmp(host, domain) == 0;
    return host_len > domain_len
        && host[host_len - domain_len - 1] == '.'
        && strcmp(host + host_len - domain_len, domain) == 0;
}

const char* llm_normalize_model_for_host(const char* host,
    const char* model)
{
    if (!model || !is_mimo_host(host))
        return model;

    if (strcmp(model, "mimo-v2-flash") == 0
        || strcmp(model, "mimo-v2-omni") == 0)
        return AGENT_LLM_MIMO_MODEL;
    if (strcmp(model, "mimo-v2-pro") == 0)
        return "mimo-v2.5-pro";
    if (strcmp(model, "mimo-v2-tts") == 0)
        return "mimo-v2.5-tts";
    return model;
}

const char* llm_normalize_host_for_key(const char* host,
    const char* api_key)
{
    if (!host || !api_key || !api_key[0]
        || (strcmp(host, AGENT_LLM_MIMO_HOST) != 0
            && strcmp(host, AGENT_LLM_MIMO_TOKEN_PLAN_HOST) != 0))
        return host;

    return strncmp(api_key, "tp-", 3) == 0
        ? AGENT_LLM_MIMO_TOKEN_PLAN_HOST
        : AGENT_LLM_MIMO_HOST;
}

static void migrate_host_config(char* host, size_t host_size,
    const char* api_key, const char* config_key)
{
    const char* normalized = llm_normalize_host_for_key(host, api_key);
    if (!normalized || strcmp(normalized, host) == 0)
        return;

    char old_host[128];
    strncpy(old_host, host, sizeof(old_host) - 1);
    old_host[sizeof(old_host) - 1] = '\0';
    strncpy(host, normalized, host_size - 1);
    host[host_size - 1] = '\0';
    claw_config_set(config_key, host);
    syslog(LOG_INFO, "[%s] Selected MiMo endpoint for key type: %s -> %s\n",
        TAG, old_host, host);
}

static void migrate_model_config(char* model, size_t model_size,
    const char* host, const char* config_key)
{
    const char* normalized = llm_normalize_model_for_host(host, model);
    if (!normalized || strcmp(normalized, model) == 0)
        return;

    char old_model[64];
    strncpy(old_model, model, sizeof(old_model) - 1);
    old_model[sizeof(old_model) - 1] = '\0';
    strncpy(model, normalized, model_size - 1);
    model[model_size - 1] = '\0';
    claw_config_set(config_key, model);
    syslog(LOG_INFO, "[%s] Migrated retired MiMo model: %s -> %s\n",
        TAG, old_model, model);
}

void add_completion_options(cJSON* body, const char* host, int max_tokens)
{
    ...
    if (is_mimo_host(host)) {
        cJSON_AddNumberToObject(body, "temperature",
            AGENT_LLM_MIMO_TEMPERATURE);
        cJSON_AddNumberToObject(body, "top_p", AGENT_LLM_MIMO_TOP_P);
        cJSON* thinking = cJSON_AddObjectToObject(body, "thinking");
        if (thinking)
            cJSON_AddStringToObject(thinking, "type",
                AGENT_LLM_MIMO_THINKING);
    }
}
...
int llm_proxy_init(void)
{
    ...
    migrate_host_config(s_llm_host, sizeof(s_llm_host), s_api_key,
        AGENT_CFG_KEY_LLM_HOST);
    if (s_vision_host[0]) {
        migrate_host_config(s_vision_host, sizeof(s_vision_host),
            s_vision_api_key[0] ? s_vision_api_key : s_api_key,
            AGENT_CFG_KEY_VISION_HOST);
    }
    migrate_model_config(s_model, sizeof(s_model), s_llm_host,
        AGENT_CFG_KEY_MODEL);
    migrate_model_config(s_vision_model, sizeof(s_vision_model),
        s_vision_host[0] ? s_vision_host : s_llm_host,
        AGENT_CFG_KEY_VISION_MODEL);
    ...
    const char* api_key)
{
    ...
    host = llm_normalize_host_for_key(host,
        api_key && api_key[0] ? api_key : s_api_key);
    ...
    int* out_status)
{
    ...
    /* MiMo documents api-key; other OpenAI-compatible APIs use Bearer. */
    char auth_header[256];
    bool mimo_auth = is_mimo_host(llm_host);
    if (mimo_auth)
    ...
    vela_header_t hdrs[] = {
        { mimo_auth ? "api-key" : "Authorization", auth_header },
    ...
    int* out_status)
{
    ...
    if (is_mimo_host(llm_host))
        snprintf(auth_header, sizeof(auth_header), "api-key: %s\r\n", api_key);
    else
    ...
    char* response_buf, size_t buf_size)
{
    ...
    add_completion_options(body, llm_host,
        is_mimo_host(llm_host) ? AGENT_LLM_MIMO_MAX_TOKENS
