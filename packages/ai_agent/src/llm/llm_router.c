...
int llm_router_init(void)
{
    ...
    for (int i = 0; i < LLM_ROUTER_MAX_BACKENDS; i++) {
        ...
        const char* normalized_host = llm_normalize_host_for_key(
            b->host, b->api_key);
        if (normalized_host && strcmp(normalized_host, b->host) != 0) {
            char old_host[128];
            strncpy(old_host, b->host, sizeof(old_host) - 1);
            old_host[sizeof(old_host) - 1] = '\0';
            strncpy(b->host, normalized_host, sizeof(b->host) - 1);
            b->host[sizeof(b->host) - 1] = '\0';
            cJSON_ReplaceItemInObject(obj, "host",
                cJSON_CreateString(b->host));
            config_migrated = true;
            syslog(LOG_INFO,
                "[%s] Selected backend %d MiMo endpoint: %s -> %s\n",
                TAG, i, old_host, b->host);
        }

        const char* normalized_model = llm_normalize_model_for_host(
            b->host, b->model);
        if (normalized_model && strcmp(normalized_model, b->model) != 0) {
            char old_model[64];
            strncpy(old_model, b->model, sizeof(old_model) - 1);
            old_model[sizeof(old_model) - 1] = '\0';
            strncpy(b->model, normalized_model, sizeof(b->model) - 1);
            b->model[sizeof(b->model) - 1] = '\0';
            cJSON_ReplaceItemInObject(obj, "model",
                cJSON_CreateString(b->model));
            config_migrated = true;
            syslog(LOG_INFO,
                "[%s] Migrated backend %d MiMo model: %s -> %s\n",
                TAG, i, old_model, b->model);
        }

        if (config_migrated) {
            char* migrated_json = cJSON_PrintUnformatted(obj);
            if (migrated_json) {
                claw_config_set(key, migrated_json);
                free(migrated_json);
            }
        }
