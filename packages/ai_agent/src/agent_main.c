...
static void* network_watch_task(void* arg)
{
    ...
    if (network_wait_connected(30000) == OK) {
...
#if AGENT_SKILL_SYNC_ENABLED
        /* Sync skills from Bitable before starting cloud channels */
        ...
        syslog(LOG_WARNING,
            "[%s] Network timeout — cloud channels remain offline; local Agent core is ready.\n",
            TAG);
        syslog(LOG_WARNING,
            "[%s] Use 'config_show' / 'set_wifi' / 'set_llm' CLI commands to configure.\n",
            TAG);
    }
...
int ai_agent_main(int argc, char* argv[])
{
     ...
     * Network-dependent cloud services are started inside the network_watch
     * thread once the link comes up. */
    ...
    /* The local Agent core and WebSocket listener do not require a network
     * address. Start them before the asynchronous Wi-Fi watcher so a first
     * boot can accept CLI configuration without requiring a restart. */
    {
        int rc = agent_loop_start();
        BOOT_LOG_RC(&t0, "P5", "agent_loop_start", rc);

        rc = ws_server_start();
        BOOT_LOG_RC(&t0, "P5", "ws_server_start", rc);
    }
