    ...
    const char* pass)
{
     ...
     * The status parser is optional because production images may omit
     * CONFIG_SYSTEM_POPEN; network_wait_connected remains the final check. */
...
#else
    syslog(LOG_INFO, "[%s] CONFIG_SYSTEM_POPEN disabled; skipping wapi status parse\n",
        TAG);
#endif
