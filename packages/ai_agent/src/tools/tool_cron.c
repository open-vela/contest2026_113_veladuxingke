    ...
    char* output, size_t output_size)
{
    ...
    /* cron_remove_job performs the authoritative locked lookup. Its error
     * result is intentionally surfaced instead of being softened into a
     * success response when a stale ID is supplied. */
    ...
    if (err == OK) {
        ...
        snprintf(output, output_size,
            "ERROR: job '%s' was not found; no job was removed. "
            "Call cron_list and use the exact current ID.", id_copy);
    }
