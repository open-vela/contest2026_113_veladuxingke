...
int tool_registry_init(void)
{
    ...
    REGISTER_TOOL("read_file",
        ...
        "For routine_mgr files under /data/routine/, use run_shell with "
        "cat instead; read_file cannot access that directory.",
    ...
    REGISTER_TOOL(
        ...
        TOOL_SCHEMA_BEGIN()
            TOOL_PARAM_STR("name", "Job name") "," TOOL_PARAM_ENUM("schedule_type", "Schedule type", "\"every\",\"at\"") "," TOOL_PARAM_NUM("interval_s", "Interval in seconds (for every)") "," TOOL_PARAM_NUM("at_epoch",
                ...
                "Tool name to execute when job fires") "," TOOL_PARAM_STR("action_args",
    ...
    REGISTER_TOOL(
        "routine_schedule_update",
        "Update one of the three persistent daily routine-manager schedule "
        "slots. Use the exact schedule_id from /data/routine/status.json; "
        "read that status with run_shell (not read_file) before updating; "
        "hour is 0-23, minute is 0-59, and text is non-empty UTF-8. "
        "The update is atomic and routine_mgr applies it on its next refresh. "
        "Do not use run_shell or file tools to edit /data/routine/.",
        TOOL_SCHEMA_BEGIN()
            TOOL_PARAM_NUM("schedule_id", "Existing slot ID: 0, 1, or 2") ","
            TOOL_PARAM_NUM("hour", "New hour, 0-23") ","
            TOOL_PARAM_NUM("minute", "New minute, 0-59") ","
            TOOL_PARAM_STR("text", "New reminder text") ","
            TOOL_PARAM_STR("expected_text",
                           "Optional current text used to reject stale edits")
            TOOL_SCHEMA_END_REQUIRED(
                "\"schedule_id\",\"hour\",\"minute\",\"text\""),
        tool_routine_schedule_update_execute);

    REGISTER_TOOL("cron_remove",
        "Remove a scheduled cron job by its exact current ID. Call cron_list "
        "first; never guess an ID. Report the error if the ID is not found.",
        TOOL_SCHEMA_BEGIN()
    ...
    REGISTER_TOOL(
        ...
        "Allowed: free, ps, df, cat, ls, curl, lvctl, etc.; direct media playback is disabled.",
        TOOL_SCHEMA_BEGIN() TOOL_PARAM_STR(
