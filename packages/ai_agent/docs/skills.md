...
`vela-desk.md` 是产品保留名称并带版本标记。设备若保留旧的
`/data/agent/skills/` 持久分区，启动时会升级缺失或低版本的产品文件，并删除已下线
的 `music-dj.md`；同目录下其它用户自定义文件不会被覆盖。
...
Skill 中可引用内置工具和 MCP 注册的外部工具。完整列表见 [tools.md](tools.md)。
